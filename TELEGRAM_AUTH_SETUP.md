...
## Шаг 5: Логин из PWA/сайта с возвратом

Чтобы логин работал не только внутри Telegram Mini App, но и в обычном браузере/PWA, используется handoff через одноразовый код:

1. В PWA/сайте кнопка "Войти" открывает Telegram Mini App через deep link (передаётся return path).
2. Mini App делает `POST /api/auth` с `initData`, сервер валидирует и возвращает `authCode`.
3. Mini App открывает ссылку обратно на сайт: `profile.html?tg_code=...`, а сайт обменивает код через `POST /api/auth/exchange` и сохраняет `tgUser` в localStorage.

Для этого добавьте переменную окружения `AUTH_CODE_SECRET` (случайная строка) в Vercel Env Vars. 

## Как это работает

1. **Авторизация**: Пользователь нажимает кнопку "Войти через Telegram" на странице профиля
